...
upgrades['5.4.5-1.el6.remi']='5.4.40-1.el6.remi'
upgrades['Django14-1.4.20-1.el6']='1.4.20-1.el6'
...
for record in data['hits']['hits']:
	if not args.one:
		table+="| " +  record['_source']['package'] 
		try:
			upgrades[record['_source']['package']]
		except KeyError:
			latest_version='unknown'
		else:
			latest_version=upgrades[record['_source']['package']]
		table+="| " +  latest_version
		#table+="| " +  upgrades[record['_source']['package']]
		table+="| " +  "FUNCTION"
		table+="| " +  record['_source']['host']  
		table+="| |\n"
	allvals.append(record['_source']['package'])
